...
class NysAct_G(Optimizer):
    ...
    def __init__(self,
        ...
        super(NysAct_G, self).__init__(params, defaults)
    ...
    def _store_input(self, module, forward_input, forward_output):
        ...
        S = torch.randn(p, rank, device=actv.device).div(p ** 0.5)       
        self.sketch_mat[module] = S
        C = torch.matmul(actv.t(), torch.matmul(actv, S).div(batch_size))
    ...
    def update_inverse(self, layer, damping, bias_correction):
        ...
        if layer in self.sketch_mat:
            ...
            damped_C = C + damping * torch.eye(C.size(0), device=C.device)
...
class NysAct_S(Optimizer):
    ...
    def __init__(self,
        ...
        defaults = dict(lr=lr, damping=damping, momentum=momentum, weight_decay=weight_decay,
                        stat_decay=stat_decay, step=0, ema_step=0)
        super(NysAct_S, self).__init__(params, defaults)
    ...
    def update_inverse(self, layer, damping, bias_correction):
        ...
        if layer in self.sketch_mat:
            ...
            Wh = torch.matmul(eigvecs, torch.matmul(torch.diag(eigvals + shift), eigvecs.T))
            L = torch.linalg.cholesky(Wh)
            X = torch.linalg.solve_triangular(L, C, upper=False, left=False)
            U, S, _ = torch.linalg.svd(X)

            rank = W.size(0)
            U = U[:, :rank]
            L = torch.reciprocal(torch.max(S.square() - shift, torch.tensor(0.0, device=C.device)) + damping)
            state['A_inv'] = torch.matmul(U, torch.matmul(torch.diag(L), U.t())) + (
                torch.diag(U.new_ones(U.size(0))) - torch.matmul(U, U.t())).div(damping)
        else:
            damped_C = C + damping * torch.eye(C.size(0), device=C.device)
