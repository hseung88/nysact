...
class Shaper(Optimizer):
    ...
    def _store_input(self,
        ...
        corrected_stat_decay = stat_decay ** self.Tcov
        ...
        moving_average(A, self.ema_A[module], corrected_stat_decay)

        if module not in self.ema_a:
            self.ema_a[module] = torch.zeros_like(actv[0])

        moving_average(actv.mean(0), self.ema_a[module], corrected_stat_decay)

    def bfgs_update(self, layer, damping):
        group = self.param_groups[0]
        stat_decay = group['stat_decay']
        corrected_stat_decay = stat_decay ** self.Tcov
        bias_correction = 1.0 - (corrected_stat_decay ** group['ema_step'])
